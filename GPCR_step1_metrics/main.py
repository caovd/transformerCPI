...
def load_tensor(file_name, dtype):
    return [dtype(d).to(device) for d in np.load(file_name + '.npy', allow_pickle=True)] #NEW - Add allow_pickle=True to avoid "ValueError: Object arrays cannot be loaded when allow_pickle=False"
...
if __name__ == "__main__":
    ...
    with det.core.init() as core_context:
        ...
        # Step 1.5 - replace dataset by shared-fs
        dir_input = ('shared_fs/' + DATASET + '/word2vec_30/')
        ...
        for epoch in range(1, iteration+1):
            ...
            loss_train = trainer.train(dataset_train, device, core_context, steps_completed)
            AUC_dev, PRC_dev = tester.test(dataset_dev, core_context, steps_completed)
